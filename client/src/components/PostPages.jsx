...
export const EditPostContainer = ({postId, title}) => {
  ...
  return (
    <EditPost post={post} title={title}/>
  )
}
    ...
    <FormSection>
      ...
      <Grid container spacing={5}>
        <Grid item xs={12} md={4}>
          <img height={200} src={post.image ? SERVER_URL + post.image : "https://picsum.photos/200?blur=10?random=" + post.id}/>
        </Grid>
        <Grid item xs={12} md={8}>
          <Typography variant={"body1"} mb={4}>{post.description}</Typography>
        </Grid>
      </Grid>
      <Box display={"flex"} justifyContent={"space-between"} mt={2}>
        <Typography variant={"subtitle2"} color={"text.secondary"} gutterBottom>
          {post.user.firstName} {post.user.lastName} - {(new Date(post.createdAt)).toLocaleDateString()}
        </Typography>
      </Box>
      <Box mt={4}>
      ...
      <Box component="form" noValidate onSubmit={handleSubmit} sx={{mt: 3}}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} ml={1}>
            <Typography variant="caption" color={"text.secondary"} mr={2}>Post image</Typography>
            <input type={"file"} name={"image"} id={"image"} defaultValue={post.image}/>
          </Grid>
          <Grid item xs={12} ml={1}>
            {post.image ? <img height={200} src={(SERVER_URL + post.image) || DEFAULT_PROFILE_PIC}
                               alt={"profile pic"}/> : "No image"}
          </Grid>
